...
{
    public partial class ctrlFrmDeleteUsrSale : Form
    {
        public ctrlFrmDeleteUsrSale()
        {
            InitializeComponent();
        }
        ...
        private void ctrlFrmDeleteUsrSale_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void ctrlFrmDeleteUsrSale_Load(object sender, EventArgs e)
        {
            txtName.Select();
        }

        private void ctrlFrmDeleteUsrSale_Leave(object sender, EventArgs e)
        {
            this.Close();
        }
        ...
        private void btnDelUsSal_Click(object sender, EventArgs e)
        {
            if(txtName.Text !="")
            {
                ...
                if(txtID.Text!="")
                {
                    ...
                    string query1 = "select user_id, name from user_details";
                    ...
                    if (dataSet.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                                //query for delete
                                if(dataSet.Tables[0].Rows[i]["name"].ToString() == usrname)
                                {
                                    lblErrName.Visible = false;
                                    if(dataSet.Tables[0].Rows[i]["user_id"].ToString() == uid)
                                    {
                                        lblErrId.Visible=false;
                                        string query2 = "delete from user_details where user_id = '" + uid + "' and name = '" + usrname + "'";
